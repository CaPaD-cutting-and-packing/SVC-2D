#ifndef __SVC_2DSPP__H
#define __SVC_2DSPP__H
...
class SVC_2DSPP : public __2DSPP { // Based on BLOCK STRUCTURES
  int res, res_best; // to hide output info
public:
  SVC_2DSPP(const __2DSPP_input& inp,
    MyStat *prms=0, MyStat *prms_all=0)
    :__2DSPP(inp), sub(inp), bl(inp), _prm_stat(prms), _prm_stat_all(prms_all) { }
  Sub_2DSPP sub;
  BL bl;
