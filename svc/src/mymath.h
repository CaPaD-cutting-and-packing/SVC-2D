...
inline double VectorProduct(iter1 a1b,iter1 a1e,iter2 a2b) {
  double sum=0;
  while (a1b < a1e)
    sum += (*(a1b++)) * (*(a2b++));
  return sum;
...
template <class D>
inline D Round(const D d) { return floor((d+0.5)); }
template <class D>
inline D round(const D d) { return floor((d+0.5)); }
template <class D>
