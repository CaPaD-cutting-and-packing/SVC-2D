...
public:
  ...
  void print_max(ostream&,const char) const;
};
...
//      cout << " Ad2: "<<d<<" at:"<<addingAt;
      ...
      if (max_[addingAt] < d-prev[addingAt])
        max_[addingAt] = d-prev[addingAt];
      ++ addingAt;
    }
    return *this;
  }
...
template <class D>  //inline // !!!
  void VectorStat<D>::print_max(ostream& os, const char dv) const {
  for (int i=0;i<accu.size();++i)
    os << max_[i] << dv;
}
