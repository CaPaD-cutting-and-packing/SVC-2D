...
(vec<unsigned> &bf,const vec<int> &sz,double SZ) {
...
void ConstructRPBitField
(vec<unsigned> &bf,
 const vec<int> &sz, const vec<int> &ub, double SZ) {
  bf.resize(int(SZ/bpw)+4);
  // MemSet(bf,0,int((zi+128)/bpw));
  bf[0]=0; bf[1]=1; // To begin from bf[1] !!
  int i;
...
// what if l[0] == 1?
void ReduceRP(vec<unsigned> &bf,double SZ) {
...
void ProduceLengths
(vec<unsigned> &bf,double SZ,vec<int> &rps) {
...
void PrintBitField(vec<unsigned> bf,double SZ) {
  unsigned * const bbff = &bf[1]; int SZint=(int)SZ;
  for (int i=0;i<=SZint; ++i)
    if (bbff[i/bpw] & (unsigned(1)<<(i%bpw)))
      cout << i << ' ';
...
// NO UPPER BNDS:
void ConstructRP
(vec<int> &sz,double SZ,vec<int> &rps) {
  vec<unsigned> bf;
...
// with UPPER BNDS:
void ConstructRP
(const vec<int> &sz,const  vec<int> &ub,
...
void ConstructReducedRP
(const vec<int> &sz, const vec<int> &ub, double SZ,vec<int> &rps) {
  vec<unsigned> bf;
  ConstructRPBitField(bf,sz,ub,SZ);
//  PrintBitField(bf,SZ);
  ReduceRP(bf,SZ);
...
int main() {
  ...
  vec<int> l(lll,lll+sizeof(lll)/sizeof(lll[0]));
  int L = 160;
  vec<int> rpl;
